...
const Header = () => {
  ...
  return (
    <header
      className={`${
        isActive ? "bg-white py-1 shadow-md" : "py-2 bg-none"
      } fixed w-full z-10`}
    >
      <div className="container mx-auto flex items-center justify-between h-full transition-all ">
        ...
        <p className="text-4xl bold">Shopping</p>
        <div className="flex gap-8 justify-between items-center">
          <Link to={"/wishlist"} className="cursor-pointer flex relative">
            <FaRegHeart className="text-2xl " />
            <div className="bg-primary absolute -right-2 -top-2 text-[12px] w-[18px] h-[18px] text-white rounded-full flex justify-center items-center">
              {wishlistAmount}
            </div>
          </Link>
